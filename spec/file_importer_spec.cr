...
describe Dotfiles::FileImporter do
  describe :import do
    it "moves file to .dotfiles folder" do
      ...
      imported_file_path = Path[Dotfiles::Utils.repository_path].join(Path[original_file.path].basename)
    ...
    it "replaces file with a symlink to new file in .dotfiles folder" do
      original_file = File.tempfile { |file| file.print("foo: 1") }
      imported_file_path = Dotfiles::Utils.repository_path.join(Path[original_file.path].basename)
